...
Public Class frmEndofthedayrep
    ...
    Private Sub Call_salesreturn()
        Try
            ...
            For i = 0 To ds.Tables("Table").Rows.Count - 1
                ...
                If (ds.Tables("Table").Rows.Item(i).Item(0) > 0) Then
                    val = (Convert.ToDouble(ds.Tables("Table").Rows.Item(i).Item(1).ToString) + Convert.ToDouble(ds.Tables("Table").Rows.Item(i).Item(2).ToString)) - Convert.ToDouble(ds.Tables("Table").Rows.Item(i).Item(3).ToString)
                Else
                    val = 0
                End If
    ...
    Private Sub Call_holding()
        Try
            If cmbSm.Text = "All" Then
                ...
                Query = "SELECT SUM(PRODQTY) AS QTY, SUM(Round(PRODPRICE,3)) AS VALUE FROM OT_POS_INVOICE_HEAD_LOG OTH, OT_POS_INVOICE_ITEM_LOG OTI WHERE OTH.INVH_SYS_ID=OTI.PROD_INVI_INVH_SYS_ID AND OTH.INVH_STATUS=4 AND INVH_COMP_CODE = '001' AND INVH_LOCN_CODE = '" & strArrLoc(0) & "' AND INVH_DT >= '" & stDateval & "' AND INVH_DT <= '" & endDateval & "' "
            Else
                Query = "SELECT SUM(PRODQTY) AS QTY, SUM(Round(PRODPRICE,3)) AS VALUE FROM OT_POS_INVOICE_HEAD_LOG OTH, OT_POS_INVOICE_ITEM_LOG OTI WHERE OTH.INVH_SYS_ID=OTI.PROD_INVI_INVH_SYS_ID AND OTH.INVH_STATUS=4 AND INVH_COMP_CODE = '001' AND INVH_LOCN_CODE = '" & strArrLoc(0) & "' AND INVH_DT >= '" & stDateval & "' AND INVH_DT <= '" & endDateval & "' AND INVH_SM_CODE = '" & cmbSm.Text & "'"
            End If
    ...
    Private Sub Call_DynamicPayment()
        Try
            ...
            For p = 0 To ds8.Tables("Table").Rows.Count - 1
