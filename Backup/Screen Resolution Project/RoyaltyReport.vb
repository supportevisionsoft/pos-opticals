...
Public Class RoyaltyReport
    ...
    Private lblCardNo As New List(Of Label)
    Private lblCardName As New List(Of Label)
    ...
    Private Sub CreateListView()
        ...
        With lstview
            ...
            .Columns.Add("Trans Date", 117, HorizontalAlignment.Center)
            .Columns.Add("Trans No", 150, HorizontalAlignment.Center)
            .Columns.Add("Card No", 150, HorizontalAlignment.Center)
            .Columns.Add("Card Name", 287, HorizontalAlignment.Center)
    ...
    Private Sub btView_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btView.Click
        Try
            ...
            If cmbSm.Text = "All" Then
                'stQuery = "SELECT to_char(INVLC_CR_DT,'dd-MM-yyyy'),INVLC_INVH_NO,INVLC_LOYALTY_NO,INVLC_LOYALTY_CARD FROM  OT_INVOICE_LOYALTY_CARD,OT_INVOICE_HEAD WHERE INVLC_COMP_CODE='001' AND INVLC_LOCN_CODE='" & strArrLoc(0) & "' AND INVLC_CR_DT >= TO_DATE('" + dtstDate.Value.ToString("dd-MM-yyyy") + "','dd-MM-yyyy')  AND INVLC_CR_DT < =TO_DATE('" + dtendDate.Value.ToString("dd-MM-yyyy") + "','dd-MM-yyyy') and INVH_FLEX_19= '" & cmbShift.Text & "' and INVH_FLEX_20= '" & cmbCounter.Text & "' and INVLC_INVH_SYS_ID = INVH_SYS_ID"
                '''stQuery = "SELECT to_char(INVLC_CR_DT,'dd-MM-yyyy'),INVLC_INVH_NO,INVLC_LOYALTY_NO,INVLC_LOYALTY_CARD FROM  OT_INVOICE_LOYALTY_CARD,OT_INVOICE_HEAD WHERE INVLC_COMP_CODE='001' AND INVLC_LOCN_CODE='" & strArrLoc(0) & "' AND INVLC_CR_DT >= '" & dtstDate.Value.ToString("dd-MMM-yy") & "000000'  AND INVLC_CR_DT <= '" & dtendDate.Value.ToString("dd-MMM-yy") & "235959'  and INVLC_INVH_SYS_ID = INVH_SYS_ID"
                stQuery = "SELECT to_char(INVLC_CR_DT,'dd-MM-yyyy'),INVLC_INVH_NO,INVLC_LOYALTY_NO,INVLC_LOYALTY_CARD FROM  OT_INVOICE_LOYALTY_CARD,OT_INVOICE_HEAD WHERE INVLC_COMP_CODE='001' AND INVLC_LOCN_CODE='" & strArrLoc(0) & "' AND INVLC_CR_DT >= TO_DATE('" + dtstDate.Value.ToString("dd-MM-yyyy") + " 000000','dd-MM-yyyy hh24miss')  AND INVLC_CR_DT <= TO_DATE('" + dtendDate.Value.ToString("dd-MM-yyyy") + "235959','dd-MM-yyyy hh24miss')  and INVLC_INVH_SYS_ID = INVH_SYS_ID"

            Else
                'stQuery = "SELECT to_char(INVLC_CR_DT,'dd-MM-yyyy'),INVLC_INVH_NO,INVLC_LOYALTY_NO,INVLC_LOYALTY_CARD FROM  OT_INVOICE_LOYALTY_CARD WHERE INVLC_COMP_CODE='001' AND INVLC_LOCN_CODE='013' AND INVLC_INVH_TXN_CODE='POSINV' AND   INVLC_CR_DT >= '07-Sep-2013'  AND INVLC_CR_DT < ='07-Oct-2013'"
                'stQuery = "SELECT to_char(INVLC_CR_DT,'dd-MM-yyyy'),INVLC_INVH_NO,INVLC_LOYALTY_NO,INVLC_LOYALTY_CARD FROM  OT_INVOICE_LOYALTY_CARD,OT_INVOICE_HEAD WHERE INVLC_COMP_CODE='001' AND INVLC_LOCN_CODE='" & strArrLoc(0) & "' AND INVLC_CR_DT >= TO_DATE('" + dtstDate.Value.ToString("dd-MM-yyyy") + "','dd-MM-yyyy')  AND INVLC_CR_DT < =TO_DATE('" + dtendDate.Value.ToString("dd-MM-yyyy") + "','dd-MM-yyyy') and INVH_FLEX_19= '" & cmbShift.Text & "' and INVH_FLEX_20= '" & cmbCounter.Text & "' and INVH_SM_CODE = '" & cmbSm.Text & "'and INVLC_INVH_SYS_ID = INVH_SYS_ID"
                '''stQuery = "SELECT to_char(INVLC_CR_DT,'dd-MM-yyyy'),INVLC_INVH_NO,INVLC_LOYALTY_NO,INVLC_LOYALTY_CARD FROM  OT_INVOICE_LOYALTY_CARD,OT_INVOICE_HEAD WHERE INVLC_COMP_CODE='001' AND INVLC_LOCN_CODE='" & strArrLoc(0) & "' AND INVLC_CR_DT >= TO_DATE('" + dtstDate.Value.ToString("dd-MM-yyyy") + "000000','dd-MM-yyyy hh24miss')  AND INVLC_CR_DT <= TO_DATE('" + dtendDate.Value.ToString("dd-MM-yyyy") + "235959','dd-MM-yyyy hh24miss') and INVH_SM_CODE = '" & cmbSm.Text & "'and INVLC_INVH_SYS_ID = INVH_SYS_ID"
                stQuery = "SELECT to_char(INVLC_CR_DT,'dd-MM-yyyy'),INVLC_INVH_NO,INVLC_LOYALTY_NO,INVLC_LOYALTY_CARD FROM  OT_INVOICE_LOYALTY_CARD,OT_INVOICE_HEAD WHERE INVLC_COMP_CODE='001' AND INVLC_LOCN_CODE='" & strArrLoc(0) & "' AND INVLC_CR_DT >= TO_DATE('" + dtstDate.Value.ToString("dd-MM-yyyy") + " 000000','dd-MM-yyyy hh24miss')" & " AND INVLC_CR_DT <= TO_DATE('" + dtendDate.Value.ToString("dd-MM-yyyy") + " 235959','dd-MM-yyyy hh24miss')" & " and INVH_SM_CODE = '" & cmbSm.Text & "'and INVLC_INVH_SYS_ID = INVH_SYS_ID"
            End If
            ...
            errLog.WriteToErrorLog("Royalty INVH Query", stQuery, "")
            'Dim ds As DataSet
            ds = db.SelectFromTableODBC(stQuery)
            count = ds.Tables("Table").Rows.Count
            invquerycount = count
            ...
            If count > 0 Then
                ...
                While count > 0
                    ...
                    lstview.Items.Add(ds.Tables("Table").Rows.Item(i).Item(0).ToString)
            ...
            If cmbSm.Text = "All" Then
                'stQuery = "SELECT to_char(INVLC_CR_DT,'dd-MM-yyyy'),INVLC_SOH_NO,INVLC_LOYALTY_NO,INVLC_LOYALTY_CARD FROM  OT_INVOICE_LOYALTY_CARD,OT_SO_HEAD WHERE INVLC_COMP_CODE='001' AND INVLC_LOCN_CODE='" & strArrLoc(0) & "' AND    INVLC_CR_DT >= TO_DATE('" + dtstDate.Value.ToString("dd-MM-yyyy") + "','dd-MM-yyyy')  AND INVLC_CR_DT < =TO_DATE('" + dtendDate.Value.ToString("dd-MM-yyyy") + "','dd-MM-yyyy') and SOH_FLEX_19= '" & cmbShift.Text & "' and SOH_FLEX_20= '" & cmbCounter.Text & "' and INVLC_SOH_SYS_ID = SOH_SYS_ID"
                '''stQuery = "SELECT to_char(INVLC_CR_DT,'dd-MM-yyyy'),INVLC_SOH_NO,INVLC_LOYALTY_NO,INVLC_LOYALTY_CARD FROM  OT_INVOICE_LOYALTY_CARD,OT_SO_HEAD WHERE INVLC_COMP_CODE='001' AND INVLC_LOCN_CODE='" & strArrLoc(0) & "' AND    INVLC_CR_DT >= TO_DATE('" + dtstDate.Value.ToString("dd-MM-yyyy") + "','dd-MM-yyyy')  AND INVLC_CR_DT < =TO_DATE('" + dtendDate.Value.ToString("dd-MM-yyyy") + "','dd-MM-yyyy') and INVLC_SOH_SYS_ID = SOH_SYS_ID"
                stQuery = "SELECT to_char(INVLC_CR_DT,'dd-MM-yyyy'),INVLC_SOH_NO,INVLC_LOYALTY_NO,INVLC_LOYALTY_CARD FROM  OT_INVOICE_LOYALTY_CARD,OT_SO_HEAD WHERE INVLC_COMP_CODE='001' AND INVLC_LOCN_CODE='" & strArrLoc(0) & "' AND    INVLC_CR_DT >= TO_DATE('" + dtstDate.Value.ToString("dd-MM-yyyy") + "000000','dd-MM-yyyy hh24miss')  AND INVLC_CR_DT < =TO_DATE('" + dtendDate.Value.ToString("dd-MM-yyyy") + "235959','dd-MM-yyyy hh24miss') and INVLC_SOH_SYS_ID = SOH_SYS_ID"
            Else
                'stQuery = "SELECT to_char(INVLC_CR_DT,'dd-MM-yyyy'),INVLC_INVH_NO,INVLC_LOYALTY_NO,INVLC_LOYALTY_CARD FROM  OT_INVOICE_LOYALTY_CARD WHERE INVLC_COMP_CODE='001' AND INVLC_LOCN_CODE='013' AND INVLC_INVH_TXN_CODE='POSINV' AND   INVLC_CR_DT >= '07-Sep-2013'  AND INVLC_CR_DT < ='07-Oct-2013'"
                'stQuery = "SELECT to_char(INVLC_CR_DT,'dd-MM-yyyy'),INVLC_SOH_NO,INVLC_LOYALTY_NO,INVLC_LOYALTY_CARD FROM  OT_INVOICE_LOYALTY_CARD,OT_SO_HEAD WHERE INVLC_COMP_CODE='001' AND INVLC_LOCN_CODE='" & strArrLoc(0) & "' AND    INVLC_CR_DT >= TO_DATE('" + dtstDate.Value.ToString("dd-MM-yyyy") + "','dd-MM-yyyy')  AND INVLC_CR_DT < =TO_DATE('" + dtendDate.Value.ToString("dd-MM-yyyy") + "','dd-MM-yyyy') and SOH_FLEX_19= '" & cmbShift.Text & "' and SOH_FLEX_20= '" & cmbCounter.Text & "' and SOH_SM_CODE = '" & cmbSm.Text & "'and INVLC_SOH_SYS_ID = SOH_SYS_ID"
                ''' stQuery = "SELECT to_char(INVLC_CR_DT,'dd-MM-yyyy'),INVLC_SOH_NO,INVLC_LOYALTY_NO,INVLC_LOYALTY_CARD FROM  OT_INVOICE_LOYALTY_CARD,OT_SO_HEAD WHERE INVLC_COMP_CODE='001' AND INVLC_LOCN_CODE='" & strArrLoc(0) & "' AND    INVLC_CR_DT >= TO_DATE('" + dtstDate.Value.ToString("dd-MM-yyyy") + "','dd-MM-yyyy')  AND INVLC_CR_DT < =TO_DATE('" + dtendDate.Value.ToString("dd-MM-yyyy") + "','dd-MM-yyyy')  and SOH_SM_CODE = '" & cmbSm.Text & "'and INVLC_SOH_SYS_ID = SOH_SYS_ID"
                stQuery = "SELECT to_char(INVLC_CR_DT,'dd-MM-yyyy'),INVLC_SOH_NO,INVLC_LOYALTY_NO,INVLC_LOYALTY_CARD FROM  OT_INVOICE_LOYALTY_CARD,OT_SO_HEAD WHERE INVLC_COMP_CODE='001' AND INVLC_LOCN_CODE='" & strArrLoc(0) & "' AND    INVLC_CR_DT >= TO_DATE('" + dtstDate.Value.ToString("dd-MM-yyyy") + "000000','dd-MM-yyyy hh24miss')  AND INVLC_CR_DT < =TO_DATE('" + dtendDate.Value.ToString("dd-MM-yyyy") + "235959','dd-MM-yyyy hh24miss')  and SOH_SM_CODE = '" & cmbSm.Text & "'and INVLC_SOH_SYS_ID = SOH_SYS_ID"
            End If
            errLog.WriteToErrorLog("Royalty SO Query", stQuery, "")
            ds = db.SelectFromTableODBC(stQuery)
            count = ds.Tables("Table").Rows.Count
            i = 0
